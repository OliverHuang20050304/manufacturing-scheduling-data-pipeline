...
| `src/parameter_conversion.py` | Converts machine rate to `C_P` and derives pairwise and weighted `C_S` values. |
...
For a usable machine rate in units per minute:

```text
C_P (minutes per unit) = 1 / machine rate
```
...
Missing machine rates are not guessed or imputed. The affected row remains in `processed_parameters.csv`, its `C_P` is blank, and its status is `missing_machine_rate`. When several rate records exist, the most recent `as_of_date` is selected and its `source_version` is retained.
